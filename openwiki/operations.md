...
The macOS job runs `cargo make test-macos-release`, then `script/release/package-macos.sh` on an ARM64 `macos-26` runner. It builds before writing credentials to disk, stages only inside `RUNNER_TEMP`, verifies the app's version, checked-in update key, canonical feed, and embedded Sparkle 2.9.4 layout, then imports the Apple Development certificate into a temporary keychain. The packager registers that keychain in the user search list and makes it the user default before it updates the key partition list. That keychain must expose exactly one valid codesigning identity, and it must be the requested identity. The release path requires an Apple Development identity label ending in `RD3D4LH465` and requires signed code to report TeamIdentifier `T54QFA7W2S`. `sign-macos-app.sh` signs the known Sparkle code graph inside-out with Hardened Runtime and `--timestamp=none`, rejecting ad hoc signatures, any other code-signing team, timestamps, and unsafe outer entitlements.
