...
This opt-in path receives only macOS system-defined events. It consumes recognized volume-up/down events for the target display, but passes them through for Mac speakers, headphones, other displays, an ambiguous LG match, or an unreadable output route; Accessibility trust is broader than that filter. Missing permission prevents enablement. If macOS disables the event tap, display communication fails, or the display connection changes, DeskHelm stops interception so later keys return to normal system handling, preserves the requested setting for recoverable failures, discards the old display session, and uses bounded fresh reads before restoring interception. Turning the feature off cancels recovery. [Architecture and Runtime](architecture-and-runtime.md#optional-keyboard-volume-control) owns the security, routing, lifecycle, and failure contract.
