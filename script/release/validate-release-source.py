#!/usr/bin/env python3
"""Validate the immutable source inputs for a DeskHelm release tag."""
...
def validate_versions(repo_root: Path, tag: str) -> tuple[str, str]:
	...
	locked_versions: dict[str, list[str]] = {name: [] for name in DESKHELM_PACKAGES}
	...
	if missing_packages:
		...
		fail(f"Cargo.lock is missing DeskHelm workspace package: {missing}")
	...
	package_swift_path = repo_root / "apps/deskhelm/macos/Package.swift"
	try:
	...
	resolved_path = repo_root / "apps/deskhelm/macos/Package.resolved"
	try:
...
def validate_git_source(
	...
	if re.fullmatch(r"[0-9a-fA-F]{40}", event_commit) is None:
		fail(f"GitHub event commit must be a full Git commit SHA: {event_commit}")
	if base_ref != "refs/remotes/origin/main":
		fail(f"release base must be canonical origin/main: {base_ref}")
	if git_output(repo_root, "cat-file", "-t", event_commit) != "commit":
		fail(f"GitHub event SHA must identify a commit directly: {event_commit}")
	...
	resolved_event_commit = git_output(repo_root, "rev-parse", "--verify", event_commit)
	head_commit = git_output(repo_root, "rev-parse", "--verify", "HEAD^{commit}")
	git_output(repo_root, "rev-parse", "--verify", f"{base_ref}^{{commit}}")

	if tag_commit != resolved_event_commit:
		fail(f"tag commit {tag_commit} does not match event commit {resolved_event_commit}")
