...
usage() {
	cat >&2 <<'USAGE'
usage: script/release/sparkle-appcast.sh --archive ZIP --appcast XML --version VERSION --tag TAG

Creates a Sparkle appcast for the signed DeskHelm macOS release archive.

Required environment:
  DESKHELM_SPARKLE_PRIVATE_ED_KEY
                          DeskHelm private EdDSA key used by Sparkle sign_update
...
if [[ -z "${DESKHELM_SPARKLE_PRIVATE_ED_KEY:-}" ]]; then
	echo "error: DESKHELM_SPARKLE_PRIVATE_ED_KEY is required to sign the update archive" >&2
	exit 1
fi
sparkle_private_key="$DESKHELM_SPARKLE_PRIVATE_ED_KEY"
unset DESKHELM_SPARKLE_PRIVATE_ED_KEY

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
sign_update="${SPARKLE_SIGN_UPDATE:-}"
if [[ -z "$sign_update" ]]; then
	sign_update="$repo_root/apps/deskhelm/macos/.build/artifacts/sparkle/Sparkle/bin/sign_update"
fi

if [[ -z "$sign_update" || ! -x "$sign_update" ]]; then
	echo "error: Sparkle sign_update tool was not found or is not executable" >&2
	echo "error: run swift package resolve/build for apps/deskhelm/macos first" >&2
	exit 1
fi
...
download_url = archive_url or f"https://github.com/acg-box/deskhelm/releases/download/{tag}/{archive}"
release_url = release_notes_url or f"https://github.com/acg-box/deskhelm/releases/tag/{tag}"
...
ET.SubElement(channel, "title").text = "DeskHelm Updates"
ET.SubElement(channel, "link").text = "https://github.com/acg-box/deskhelm/releases"
ET.SubElement(channel, "description").text = "DeskHelm macOS app updates."
